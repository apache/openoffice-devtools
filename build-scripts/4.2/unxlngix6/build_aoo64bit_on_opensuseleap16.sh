...
while true; do
	case "$1" in
		...
		"--system-python" ) AOO_PYTHON="--with-system-python"; shift ;;
                "--sanitizer" ) AOO_DEBUG="$AOO_DEBUG --enable-sanitizer=yes --disable-unit-tests"; shift ;;
		"--" ) shift; break ;;
		"" ) break ;;
...
if [ "$AOO_SKIP_CONFIG" != "yes" ]; then
	...
	--with-jdk-home=$HOME/java/jdk8u472-b08 \
